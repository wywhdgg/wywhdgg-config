...
public class EnvController {
    ...
    public String index(Model model) {
        ...
        List<ConfEnv> list = confEnvService.findAll();
    ...
    public Result<String> save(ConfEnv xxlConfEnv) {
        ...
        // valid repeat
        ConfEnv existEnv = confEnvService.load(xxlConfEnv.getEnv());
        if (existEnv != null) {
            return new Result<String>(500, "Env已存在，请勿重复添加");
        }

        int ret = confEnvService.save(xxlConfEnv);
    ...
    public Result<String> update(ConfEnv xxlConfEnv) {
        ...
        int ret = confEnvService.update(xxlConfEnv);
        return (ret > 0) ? Result.SUCCESS : Result.FAIL;
    }
    ...
    public Result<String> remove(String env) {
        ...
        // valid
        int list_count = confNodeService.pageListCount(0, 10, env, null, null);
        if (list_count > 0) {
            return new Result<String>(500, "拒绝删除，该Env下存在配置数据");
        }

        // valid can not be empty
        List<ConfEnv> allList = confEnvService.findAll();
        if (allList.size() == 1) {
            return new Result<String>(500, "拒绝删除, 需要至少预留一个Env");
        }

        int ret = confEnvService.delete(env);
        return (ret > 0) ? Result.SUCCESS : Result.FAIL;
    }
}
